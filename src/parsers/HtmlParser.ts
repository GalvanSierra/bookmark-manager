import { DEFAULT_FOLDER, HTML_TEMPLATE } from '@/config/constant';
 ...
 */
export class HtmlParser implements IBookmarkParser {
  /**
  ...
  public parse(content: string): BookmarkSchema[] {
    ...
    for (const rawLine of lines) {
      ...
      if (folderMatch) {
        ...
        if (folderName === DEFAULT_FOLDER) continue;
      ...
      if (bookmarkMatch) {
        ...
        const folder = folderStack.length > 0 ? folderStack.join(' > ') : DEFAULT_FOLDER;
  ...
  private buildFolderTree(bookmarks: Bookmark[]): FolderNode {
    ...
    bookmarks.forEach((bookmark) => {
      ...
      for (const part of parts) {
        ...
        currentNode = currentNode.children.get(part) || currentNode;
      }
